...
class GuokrSpider(scrapy.Spider):
    ...
    def parse_other(self, response):
        ...
        titlefield = response.xpath(
            '//div[@class="Article__FixedSideTool-sc-1dunux7-1 fKbQrP"]/ul/li[1]/a/text()').extract_first()
        print("***领域为:{}".format(titlefield))
        titlefields = response.xpath(
            '//div[@class="Article__FixedSideTool-sc-1dunux7-1 fKbQrP"]/ul/li[2]/a/text()').extract_first()

        floor_list = response.xpath(
            '//div[@class="layout__Skeleton-zgzfsa-3 styled__ArticleContent-sc-1ctyfcr-4 bhhvjp"]')
        content = []
